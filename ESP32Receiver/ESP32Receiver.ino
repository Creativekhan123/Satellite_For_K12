    ...
    * { box-sizing: border-box; }
    ...
    body::after {
      ...
      background: repeating-linear-gradient(
        ...
        transparent,
        transparent 2px,
        rgba(0,255,234,0.018) 2px,
        rgba(0,255,234,0.018) 4px
      );
    ...
    /* ─── Telemetry card live-update pulse ───────────────────────── */
    @keyframes cardPulse {
      0%   { box-shadow: inset 0 0 20px rgba(0,0,0,0.55); }
      35%  { box-shadow: inset 0 0 20px rgba(0,0,0,0.55), 0 0 0 3px rgba(0,255,234,0.22); }
      100% { box-shadow: inset 0 0 20px rgba(0,0,0,0.55); }
    }
    .card.data-tick { animation: cardPulse 0.55s ease-out; }

    /* ─── Packet Waterfall Timeline ──────────────────────────────── */
    #pkt-waterfall {
      display: flex;
      gap: 2px;
      align-items: center;
...
<body>
  ...
  <script>
    ...
    // ─── Packet Waterfall (last 40 packets visual) ────────────────
    const WF_MAX = 40;
    const wfHistory = [];
    function wfPush(ok) {
      wfHistory.push(ok);
      if (wfHistory.length > WF_MAX) wfHistory.shift();
      const c = document.getElementById('pkt-waterfall');
      if (!c) return;
      c.innerHTML = '';
      wfHistory.forEach(v => {
        const d = document.createElement('div');
        d.className = 'wf-dot ' + (v ? 'ok' : 'miss');
        c.appendChild(d);
      });
    }
      ...
      fetch('/data')
        ...
        .then(d=>{
          ...
          if (d.connected) {
            ...
            // Waterfall: mark packet received OK
            wfPush(true);

            // Pulse data cards on every live update (visual confirmation)
            document.querySelectorAll('#data-grid .card').forEach(c => {
              c.classList.remove('data-tick');
              void c.offsetWidth; // force reflow to restart animation
              c.classList.add('data-tick');
            });
            ...
            // ESP32 confirmed no RF packets for > 8 seconds
            wfPush(false); // mark dropped in waterfall
